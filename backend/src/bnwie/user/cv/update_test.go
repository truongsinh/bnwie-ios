...
func TestUserUpdateError(t *testing.T) {
	...
	Expect(err).To(BeNil())
	Expect(string(body)).To(MatchJSON(`{"Code":-1,"Debug":"Struct:ExtendedData\nField validation for \"Description\" failed on the \"required\" tag\nField validation for \"Provider\" failed on the \"required\" tag\nField validation for \"Consumer\" failed on the \"required\" tag","Error":"ExtendedData required"}`))
}

func TestUserUpdateSucess(t *testing.T) {
	RegisterTestingT(t)
	...
	body, err := ioutil.ReadAll(res.Body)
	Expect(err).To(BeNil())
	Expect(string(body)).To(MatchJSON(`{"Status":"Updated"}`))
}

func TestUserUpdateCtrl(t *testing.T) {
	RegisterTestingT(t)
